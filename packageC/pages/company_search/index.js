// packageC/pages/company_search/index.js
...
Page({
  ...
  async onLoad(options) {
    ...
    that.setData({
      ...
      enterpriseId: options.enterpriseId?options.enterpriseId: wx.getStorageSync('enterpriseIdStorage'), //商铺的企业Id
      regionId: options.regionId?options.regionId: wx.getStorageSync('regionIdStorage'),
    })
    ...
    if (showHistory) {
      ...
      this.selectComponent('#goods').data.drugTypeId = options.drugTypeId;
      that.setData({
  ...
  onUnload: function () {
    ...
    wx.removeStorageSync('findHotdrug_data2');
    wx.removeStorageSync('enterpriseIdStorage');
    wx.removeStorageSync('regionIdStorage')
  },
    ...
    if (areaData) {
      ...
      if (res.data.returnCode == "ERR_0000") {
        ...
        let resultData = res.data.data.filter(item => {
          return that.data.source == "company" && that.data.enterpriseId == item.enterpriseId;
        })
        that.setData({
  ...
  getHistroy() {
    ...
    if (wx.getStorageSync("searchHistrory_data" + that.data.enterpriseId) != "") {
      that.setData({
        historyData: wx.getStorageSync('searchHistrory_data' + that.data.enterpriseId)
      })
    }
  },
  ...
  async findHotdrug() {
    let that = this;
    let hotData = wx.getStorageSync('findHotdrug_data2');
    if (hotData != "") {
      that.setData({
        hotData
      })
    } else {
      let res = await wx.$http.post('/ydh/mall/stores/findHotdrug', {
        pageNum: 1,
        pageSize: 50,
        enterpriseId: that.data.enterpriseId?that.data.enterpriseId:wx.getStorageSync('enterpriseIdStorage')
      }, {
      ...
      if (res.data.returnCode == "ERR_0000") {
        ...
        wx.setStorageSync('findHotdrug_data2', res.data.data.list);
      }
    }
  },
  ...
  search() {
    ...
    that.selectComponent('#goods').data.regionId = this.data.regionId ? this.data.regionId : wx.getStorageSync('regionIdStorage')?wx.getStorageSync('regionIdStorage'):0;
  },
  ...
  clickYP(e) {
    ...
    if (that.data.fl_currentIndex == -1) {
      ...
      that.selectComponent('#goods').data.categoryId = '';
      that.selectComponent('#goods').data.drugTypeId = that.data.cid; //二级分类id
    }
  },


  //插入到历史搜索数据  根据企业Id进行缓存数据
  /**
   * 切换企业，店铺页面搜索历史数据跟随企业切换
   */
  addHistory(name) {
    let that = this;
    let arr = wx.getStorageSync('searchHistrory_data' + that.data.enterpriseId);
    if (arr != "") {
      ...
      if (name != "") {
        ...
        //搜搜历史数据不能超过10条
        arr = arr.slice(0, 10) //开始索引为1 结束索引为10(不包括10
        wx.setStorageSync("searchHistrory_data" + that.data.enterpriseId, arr);
      } else {
        that.setData({
          showHistory: false,
          ['ruleForm.keyWord']: ""
      ...
      if (name != "") {
        wx.setStorageSync('searchHistrory_data' + that.data.enterpriseId, [{
          name: name
        }])
      }
    }
    if (arr != "") {
      ...
      that.selectComponent('#goods').data.keyWord = wx.getStorageSync('searchHistrory_data' + that.data.enterpriseId)[0].name;
  ...
  clear() {
    let that = this;
    wx.removeStorageSync('searchHistrory_data' + that.data.enterpriseId);
    that.setData({
      historyData: []
    })
  },
